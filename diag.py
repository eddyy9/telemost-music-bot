# -*- coding: utf-8 -*-
"""
Диагностика: почему бота не слышно в Телемосте.

Проверяет по порядку:
  1. запрет микрофона на уровне Windows (реестр);
  2. видит ли Python виртуальный кабель;
  3. видит ли его сам браузер и отдаёт ли микрофон вообще.

Третий пункт открывает браузер на настоящем домене Телемоста (страница
подменяется локально), поэтому проверяется ровно то разрешение, которое
спрашивает настоящий Телемост.
"""
