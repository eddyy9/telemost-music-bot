# -*- coding: utf-8 -*-
"""
Аудио-движок: yt-dlp -> ffmpeg -> буфер -> громкость и лимитер -> кабель.

Весь тракт идёт в float32, поэтому целочисленного округления нет ни на одном
шаге. Пики поджимаются мягким лимитером вместо жёсткой обрезки: обрезка даёт
квадратную волну, то есть слышимый хрип, на который кодек ещё и тратит биты.

Между декодером и звуковой картой стоит буфер на несколько секунд — иначе
любая заминка сети или нагрузка от игры оборачивается щелчком.
"""
